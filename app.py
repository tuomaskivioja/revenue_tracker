...
def create_table_for_user(username):
    ...
    # Dynamically create a table based on the username, with both sales and clicks
...
def increment_sale(username, video_clicked):
    ...
    if row:
        ...
        # If the video doesn't exist, insert it with a sale count of 1 and click count of 0
        c.execute(f'INSERT INTO {table_name} (video_clicked, sale_count, click_count) VALUES (?, ?, ?)', (video_clicked, 1, 0))

    conn.commit()
    conn.close()

# Function to increment clicks for a specific video in a user's table
def increment_click(username, video_clicked):
    conn = sqlite3.connect('sales.db')
    c = conn.cursor()
    ...
    # Check if the video already exists in the user's table
    c.execute(f'SELECT click_count FROM {table_name} WHERE video_clicked = ?', (video_clicked,))
    row = c.fetchone()

    if row:
        # If the video exists, increment the click count
        new_count = row[0] + 1
        c.execute(f'UPDATE {table_name} SET click_count = ? WHERE video_clicked = ?', (new_count, video_clicked))
    else:
        # If the video doesn't exist, insert it with a click count of 1 and sale count of 0
        c.execute(f'INSERT INTO {table_name} (video_clicked, sale_count, click_count) VALUES (?, ?, ?)', (video_clicked, 0, 1))

    conn.commit()
    conn.close()

@app.route('/api/video-click/<username>', methods=['POST'])
def handle_video_click(username):
    ...
    try:
        ...
        # Increment click for the video clicked in the user's table
        increment_click(username, video_clicked)
        return jsonify({'message': f'Click recorded for video: {video_clicked} in table {username}'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500


# View all sales and clicks data for a specific user (for testing/debugging purposes)
...
def get_sales(username):
    ...
    try:
        c.execute(f'SELECT video_clicked, sale_count, click_count FROM {table_name}')
        sales = c.fetchall()
